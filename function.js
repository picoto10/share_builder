var estrategias = [
  estrategia1 = {
    nombre: "UNA Y OTRA VEZ ARTE  Y JUEGO PARA APRENDER",
    url: "https://sites.google.com/view/femsaestrategia1/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","autocontrol","perseveracia","confianza","individualidad","motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia2 = {
    nombre: "RECONOCIMIENTO Y EXPRESIÓN DE LAS EMOCIONES EN ESTUDIANTES DE GRADO 5",
    url: "https://sites.google.com/view/femsaestrategia2/inicio",
    componente: "Conciencia de sí mismo",
    tipo: 1,
    tag: ["todas","autocontrol","comunicacion","aceptacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia3 = {
    nombre: "FAMILIA – COMUNICACIÓN ASERTIVA",
    url: "https://sites.google.com/view/femsaestrategia3/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia4 = {
    nombre: "DESCUBRIENDO EL MUNDO EN FAMILIA",
    url: "https://sites.google.com/view/femsaestrategia4/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor","comunicacion","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia5 = {
    nombre: "LITERATIANDO ANDO CON MIS EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia5/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol","perseverancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia6 = {
    nombre: "COMUNICÁNDONOS DE MANERA EFECTIVA PARA EL APRENDIZAJE Y BIENESTAR DE NUESTROS NIÑOS Y NIÑAS",
    url: "https://sites.google.com/view/femsaestrategia6/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia7 = {
    nombre: "EL CARRUSEL DE LAS EMOCIONES PARENTALES",
    url: "https://sites.google.com/view/femsaestrategia7/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","autocontrol","paciencia","atencionplena","tolerancia","respeto","amor","comunicacion","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia8 = {
    nombre: "LOS JÓVENES EXPLORAN SUS EMOCIONES DESDE EL ARTE",
    url: "https://sites.google.com/view/femsaestrategia8/inicio",
    componente: "Conciencia de sí mismo",
    tipo: 3,
    tag: ["todas","confianza", "individualidad", "motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia9 = {
    nombre: "JUGANDO Y EXPLORANDO LA COMUNICACIÓN VAMOS MEJORANDO",
    url: "https://sites.google.com/view/femsaestrategia9/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia10 = {
    nombre: "HACIA UNA CONVIVENCIA ARMÓNICA DE LOS NIÑOS Y NIÑAS",
    url: "https://sites.google.com/view/femsaestrategia10/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","perseverancia","comunicacion","empatia","atencionplena","motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia11 = {
    nombre: "FOMENTAR Y FORTALECER ACOMPAÑAMIENTO DE LAS FAMILIAS EN LOS PROCESOS PEDAGÓGICOS DE LOS HIJOS",
    url: "https://sites.google.com/view/femsaestrategia12/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia12 = {
    nombre: "JUGANDO, JUGANDO... VAMOS TODOS AVANZANDO, APRENDIENDO Y SOCIALIZANDO",
    url: "https://sites.google.com/view/femsaestrategia13/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia13 = {
    nombre: "+ PARTICIPACIÓN - DISTANCIA",
    url: "https://sites.google.com/view/femsaestrategia14/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia14 = {
    nombre: "ME RECONOZCO Y ME EXPRESO",
    url: "https://sites.google.com/view/femsaestrategia15/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia15 = {
    nombre: "AUTORREGULACIÓN EMOCIONAL EN NIÑOS DE 6 AÑOS DE EDAD",
    url: "https://sites.google.com/view/femsaestrategia16/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia16 = {
    nombre: "EL ARTE, UNA ESTRATEGIA PARA INTEGRAR A LA FAMILIA EN EL APRENDIZAJE DE NUESTROS ESTUDIANTES",
    url: "https://sites.google.com/view/femsaestrategia17/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia17 = {
    nombre: "LÚDICA EN VALORES EDUCATIVOS A TRAVÉS DE ENTORNOS VIRTUALES",
    url: "https://sites.google.com/view/femsaestrategia19/inicio",
    componente: "Conciencia Social",
    tipo: 4,
    tag: ["todas","autocontrol","empatia","respeto"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia18 = {
    nombre: "CONEXIÓN DE VIDA",
    url: "https://sites.google.com/view/femsaestrategia18/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion", "resiliencia","empatia","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia19 = {
    nombre: "JUEGO Y APRENDO CON TODOS",
    url: "https://sites.google.com/view/femsaestrategia20/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["ninguna"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia20 = {
    nombre: "EL ARTE Y LA LITERATURA COMO ESTRATEGIA DE CREACIÓN DE ESPACIOS PEDAGÓGICOS Y FAMILIARES MÁS FELICES",
    url: "https://sites.google.com/view/femsaestrategia21/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol","resiliencia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia21 = {
    nombre: "RED ESCUELA - FAMILIA – CIUDAD",
    url: "https://sites.google.com/view/femsaestrategia71/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol","paciencia","resiliencia","confianza"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia22 = {
    nombre: "LEGOMANÍA, UN JUEGO PARA DESPARCHARTE EN FAMILIA",
    url: "https://sites.google.com/view/femsaestrategia22/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol", "paciencia", "resiliencia","confianza"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia23 = {
    nombre: "¡SI UNA BUENA COMUNICACIÓN QUIERES TRANSMITIR, ESTA ESTRATEGIA DEBES INCLUIR!",
    url: "https://sites.google.com/view/femsaestrategia23/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","paciencia","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia24 = {
    nombre: "CON ATENCIÓN TRABAJO MEJOR",
    url: "https://sites.google.com/view/femsaestrategia24/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","cooperacion", "amor", "comunicacion", "empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia25 = {
    nombre: "EXPRESANDO Y JUGANDO ANDO",
    url: "https://sites.google.com/view/femsaestrategia25/inicio",
    componente: "Conciencia Social",
    tipo: 4,
    tag: ["todas","respecto","tolerancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia26 = {
    nombre: "MOTIVARTE MAESTRO",
    url: "https://sites.google.com/view/femsaestrategia26/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","resiliencia","perseverancia","motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia27 = {
    nombre: "¿ME ACOMPAÑAS Y TE COMPROMETES A APRENDER CONMIGO?",
    url: "https://sites.google.com/view/femsaestrategia27/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","empatia","respeto"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia28 = {
    nombre: "CONSTRUYENDO JUNTOS NUESTRAS NORMAS EN CASA",
    url: "https://sites.google.com/view/femsaestrategia28/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","autocontrol","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia29 = {
    nombre: "LEYENDO Y EXPLORANDO, VOY CRECIENDO Y EL MUNDO MEJORANDO",
    url: "https://sites.google.com/view/femsaestrategia30/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","perseverancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia30 = {
    nombre: "PALABRAS QUE ABRAZAN",
    url: "https://sites.google.com/view/femsaestrategia31/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","resiliencia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia31 = {
    nombre: "TEJIENDO VÍNCULOS",
    url: "https://sites.google.com/view/femsaestrategia32/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor","motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia32 = {
    nombre: "¿QUÉ ESTRATEGIAS LÚDICAS PODEMOS IMPLEMENTAR PARA DISMINUIR LA BAJA TOLERANCIA A LA FRUSTRACIÓN EN NIÑOS Y NIÑAS DE 6 AÑOS DE LAS INSTITUCIONES EDUCATIVAS?",
    url: "https://sites.google.com/view/femsaestrategia33/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","resiliencia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia33 = {
    nombre: "ARTE CON AMOR Y EN MI CASA DISFRUTO DEL COLOR",
    url: "https://sites.google.com/view/femsaestrategia34/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","cooperacion","resiliencia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia34 = {
    nombre: "RISAS DE COLORES, LLUVIA DE EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia35/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","empatia","motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia35 = {
    nombre: "ARTE COMO MEDIADOR PARA EL FORTALECIMIENTO DE LA IDENTIDAD",
    url: "https://sites.google.com/view/femsaestrategia72/inicio",
    componente: "Conciencia de sí mismo",
    tipo: 3,
    tag: ["todas","aceptacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia36 = {
    nombre: "«EL JUEGO» LA MEJOR MEDIACIÓN",
    url: "https://sites.google.com/view/femsaestrategia36/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","comunicacion","perserverancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia37 = {
    nombre: "LA COMUNICACIÓN REFLEJO DE AMOR Y PUENTE ENTRE FAMILIA Y DOCENTE",
    url: "https://sites.google.com/view/femsaestrategia37/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia38 = {
    nombre: "MANEJA TU TIEMPO, MANEJA TU SALUD",
    url: "https://sites.google.com/view/femsaestrategia38/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia39 = {
    nombre: "«EXPRESSARTE», ME ATREVO, TE ATREVES, EXPRESÉMONOS Y CONOZCÁMONOS MEJOR",
    url: "https://sites.google.com/view/femsaestrategia39/inicio",
    componente: "Conciencia social",
    tipo: 4,
    tag: ["todas","autocontrol","tolerancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia40 = {
    nombre: "EL JUEGO Y LA LITERATURA: ESTRATEGIAS POTENCIADORAS DE LA ESCUCHA ACTIVA",
    url: "https://sites.google.com/view/femsaestrategia40/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","cooperacion","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia41 = {
    nombre: "UNIDOS ACTIVOS",
    url: "https://sites.google.com/view/femsaestrategia41/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","cooperacion","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia42 = {
    nombre: "APRENDIENDO A EXPRESAR MIS EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia42/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia43 = {
    nombre: "DESCUBRO E IDENTIFICO MIS EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia43/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","autocontrol","confianza"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia44 = {
    nombre: "CONOCIENDO EL DESARROLLO EMOCIONAL EN LOS NIÑOS A TRAVÉS DE ESTRATEGIAS DIDÁCTICAS TRANSFORMANDO REALIDADES POR MEDIO DE LA LITERATURA",
    url: "https://sites.google.com/view/femsaestrategia44/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia45 = {
    nombre: "LAS EMOCIONES A TRAVÉS DEL JUEGO",
    url: "https://sites.google.com/view/femsaestrategia45/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia46 = {
    nombre: "¡MI VIDA ES UN CUENTO!",
    url: "https://sites.google.com/view/femsaestrategia46/inicio",
    componente: "Conciencia de sí mismo",
    tipo: 3,
    tag: ["todas","aceptacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia47 = {
    nombre: "JUEGO, CREO EN MÍ Y SOY CAPAZ",
    url: "https://sites.google.com/view/femsaestrategia47/inicio",
    componente: "Conciencia de sí mismo",
    tipo: 3,
    tag: ["todas","individualidad","confianza"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia48 = {
    nombre: "EXPLORACIÓN ACTIVA",
    url: "https://sites.google.com/view/femsaestrategia48/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia49 = {
    nombre: "JUGANDO APRENDEMOS A CUIDARNOS DEL CORONAVIRUS",
    url: "https://sites.google.com/view/femsaestrategia49/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","resiliencia","atencionplena"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia50 = {
    nombre: "FAMILIA Y ESCUELA EN UN VIAJE POR EL MUNDO DE LAS EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia50/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia51 = {
    nombre: "PINTAMOS LAS EMOCIONES PARA COMUNICAR NUESTRA HISTORIA",
    url: "https://sites.google.com/view/femsaestrategia51/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia52 = {
    nombre: "EL CARRUSEL DE LAS EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia52/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["ninguna"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia53 = {
    nombre: "SEMBRANDO Y CULTIVANDO EL BUEN TRATO",
    url: "https://sites.google.com/view/femsaestrategia54/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia54 = {
    nombre: "A TERTULIAR EN FAMILIA",
    url: "https://sites.google.com/view/femsaestrategia55/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 1,
    tag: ["todas","amor","comunicacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia55 = {
    nombre: "MI ALMA VUELA CUANDO CONTROLO MIS IMPULSOS",
    url: "https://sites.google.com/view/femsaestrategia56/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","comunicación","autocontrol"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia56 = {
    nombre: "¡JUGANDO y JUGANDO, MIS EMOCIONES VOY ENCONTRANDO!",
    url: "https://sites.google.com/view/femsaestrategia57/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","autocontrol"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia57 = {
    nombre: "LEEMOCIÓNATE",
    url: "https://sites.google.com/view/femsaestrategia58/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","resiliencia","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia58 = {
    nombre: "EL TREN DE LAS EMOCIONES",
    url: "https://sites.google.com/view/femsaestrategia59/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","perseveracia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia59 = {
    nombre: "ESPACIOS ARTÍSTICOS PARA JÓVENES EN TIEMPO DE PANDEMIA",
    url: "https://sites.google.com/view/femsaestrategia67/inicio",
    componente: "Autogestión",
    tipo: 1,
    tag: ["todas","autocontrol","resiliencia","atencionplena"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia60 = {
    nombre: "LA MÁGICA AVENTURA DE LA COMUNICACIÓN",
    url: "https://sites.google.com/view/femsaestrategia61/inicio",
    componente: "Autogestión",
    tipo: 2,
    tag: ["todas","atencionplena"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia61 = {
    nombre: "ESTRATEGIA DE INCLUSIÓN EN PROCESOS PARA PADRES DE PRIMERA INFANCIA A TRAVÉS DEL ARTE",
    url: "https://sites.google.com/view/femsaestrategia62/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 2,
    tag: ["todas","amor","comunicacion","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia62 = {
    nombre: "EL ARTE COMO ESPEJO DEL YO: FORTALECIENDO EL AUTORRECONOCIMIENTO EN LA INFANCIA",
    url: "https://sites.google.com/view/femsaestrategia63/inicio",
    componente: "Conocimiento de sí mismo",
    tipo: 3,
    tag: ["todas","aceptacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia63 = {
    nombre: "EL JUEGO COMO MEDIDOR PARA AFIANZAR LAS HABILIDADES SOCIOEMOCIONALES EN LOS NIÑOS Y NIÑAS DE 7 A 10 AÑOS",
    url: "https://sites.google.com/view/femsaestrategia64/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 3,
    tag: ["todas","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia64 = {
    nombre: "EL JUEGO PARA MAYOR DISFRUTE DE LA VIDA",
    url: "https://sites.google.com/view/femsaestrategia65/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 3,
    tag: ["todas","amor"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia65 = {
    nombre: "RE-CREANDO MIS VIVENCIAS",
    url: "https://sites.google.com/view/femsaestrategia66/inicio",
    componente: "Conocimiento de sí mismo",
    tipo: 3,
    tag: ["todas","aceptacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia66 = {
    nombre: "LA CONTAMINACIÓN AMBIENTAL Y SU IMPACTO SOCIAL EN EL DESARROLLO SOSTENIBLE DEL PLANETA",
    url: "https://sites.google.com/view/la-contaminacion-ambiental-y-s/crea",
    componente: "Contaminación ambiental",
    tipo: 3,
    tag: ["todas","resiliencia","respecto"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia67 = {
    nombre: "IESA EMPATIZA CON PLAYA BLANCA",
    url: "https://sites.google.com/view/escribir-nombre-breve-de-la-es/experimenta",
    componente: "Concervación recursos marinos",
    tipo: 3,
    tag: ["todas","respecto","tolerancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia68 = {
    nombre: "ESPACIOS DE ENTRETENIMIENTO PARA UN APRENDIZAJE LÚDICO Y CON CALIDAD",
    url: "https://sites.google.com/view/espacios-para-un-aprendizaje-l/experimenta",
    componente: "Desarrollo de competencia básicas: lectura y matemáticas",
    tipo: 3,
    tag: ["todas","aceptacion","confianza"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia69 = {
    nombre: "ME RELACIONO Y ME COMUNICO EXPRESANDO MIS EMOCIONES",
    url: "https://sites.google.com/view/gr-1-sub1-ras21-2-estrategia01/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 3,
    tag: ["todas","comunicación","cooperacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia70 = {
    nombre: "DESARROLLO DE LA EMPATÍA EN NIÑOS Y NIÑAS DE 4 A 6 AÑOS A TRAVÉS DE LAS HABILIDADES SOCIALES  Y LAS ACTIVIDADES RECTORAS E INTENCIONADAS",
    url: "https://sites.google.com/view/gr-1-sub2-ras21-2-estrategia02/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 3,
    tag: ["todas","empatia","aceptacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia71 = {
    nombre: "EL MALETÍN VIAJERO",
    url: "https://sites.google.com/view/gr-2-sub1-ras21-2-estrategia04/inicio",
    componente: "Autogestión",
    tipo: 3,
    tag: ["todas","autogestion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia72 = {
    nombre: "ESCUCHA ACTIVA COMO MEDIO PARA LOGRAR UNA COMUNICACIÓN ASERTIVA",
    url: "https://sites.google.com/view/gr-2-sub2-ras21-2-estrategia05/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 3,
    tag: ["todas","comunicacion","tolerancia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia73 = {
    nombre: "JUGANDO AL AJEDREZ INCENTIVAMOS NUESTRO POTENCIAL CREATIVO",
    url: "https://sites.google.com/view/gr3sub4ras/estado",
    componente: "Conocimiento de sí mismo",
    tipo: 3,
    tag: ["todas","aceptacion","individualidad"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia74 = {
    nombre: "MOTIVA2 A RETORNAR",
    url: "https://sites.google.com/view/la-motivacin/estado",
    componente: "Conocimiento de sí mismo",
    tipo: 3,
    tag: ["todas","aceptacion","motivacion"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  },
  estrategia75 = {
    nombre: "APRENDIENDO A COMPARTIR Y DISFRUTAR DE LOS DEMAS",
    url: "https://sites.google.com/view/gr-4-sub3-ras21-2-estrategia12/inicio",
    componente: "Habilidades para relacionarse",
    tipo: 3,
    tag: ["todas","aceptacion","empatia"],
    cargar: function(){
      return ("Estrategia: "+this.nombre);
    }
  }
];

var contenedor = document.getElementById("container");

function mostrar(tag){
  var count = 0;
  contenedor.innerHTML = "";
  if(tag == 'todas'){
    var arreglo = aleatorias();
    for (var i = 0; i < arreglo.length; i++) {
      count++;
      var pos = arreglo[i];
      construir(estrategias[pos]);
    }
  }else{
    for (var i = 0; i < estrategias.length; i++) {
      if (estrategias[i].tag.indexOf(tag) != -1) {
        count++;
        construir(estrategias[i]);
        console.log(estrategias[i].cargar());
      }
    }
  }
  if (count == 0) {
    contenedor.innerHTML = '<h1>Opps... No existen estrategias planteadas para esta habilidad</h1>';
  }
}

function construir(estrategia){
  contenedor.innerHTML += `
  <div id="card" class="card">
  <div id="card-header" class="card-header">
    `+acortar(estrategia.nombre)+`
  </div>
  <div id="card-content" class="card-content">
  <span class="`+codigos(estrategia.tipo)+` etiqueta">`+estrategia.componente+`</span>
  </div>
  <div id="card-footer" class="card-footer">
  <a href="#" id="card-button" class="card-button" onclick = "abrirEstrategia('`+estrategia.url+`')">Ver estrategia</a>
  </div>
  </div>
  `;
}

function aleatorias(){
  var arreglo = new Array(9);
  var count = 0;
  while(count < arreglo.length){
    const random = Math.floor(Math.random()*73);
    if(arreglo.indexOf(random) == -1){
      arreglo[count] = random;
      count++;
    }
  }
  return arreglo;
}

function codigos(componente){
  switch (componente) {
    case 1:
      return "tipo1";
    case 2:
      return "tipo2";
    case 3:
      return "tipo3";
    case 4:
      return "tipo4";
    default:
      return "tipo1";
  }
}

function acortar(palabra){
  if(palabra.length >= 32){
    return palabra.substring(0,45)+"...";
  }else{
    return palabra+'<br>&nbsp;';
  }
}

function abrirEstrategia(url){
  window.open(url,"_blank");
}

window.onload = function(){
  mostrar('todas');
}
